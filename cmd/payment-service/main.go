...
//   - Redis Integration (payment query cache)
...
import (
	...
	"github.com/rohithsandiri/Summer_Project/services/payment-service/handler"
	"github.com/rohithsandiri/Summer_Project/services/payment-service/repository"
	"github.com/rohithsandiri/Summer_Project/services/payment-service/routes"
	"github.com/rohithsandiri/Summer_Project/services/payment-service/service"
)

func main() {
	// ── 1. Configuration (fail-fast validation) ───────────────────────────────
	cfg, err := config.Load("payment-service", config.LoadOptions{
		RequireRedis: true,
	})
	if err != nil {
	...
	// ── 6. Business Metrics ───────────────────────────────────────────────────
	bm := metrics.NewPaymentMetrics(m.Registry())
	...
	healthHandler.AddCheck("payment_gateway", health.OKCheck("simulated"))
