// services/payment-service/routes/routes.go
//
...
// Register wires all payment routes onto the provided mux.
func Register(
	mux *http.ServeMux,
	...
	mux.Handle("POST /payment/process", chain(h.Process))
	mux.Handle("POST /payment/refund", chain(h.Refund))
	mux.Handle("GET /payment/{id}", chain(h.GetPayment))
